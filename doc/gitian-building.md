...
*Setup instructions for a gitian build of Lightcoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Lightcoin
...
to the Lightcoin GitHub release page.
...
- [Building Lightcoin Core](#building-Lightcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/Lightcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/Lightcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/Lightcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/Lightcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/Lightcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/Lightcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/LightcoinNodes/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@Lightcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/Lightcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/Lightcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/Lightcoin-osx-*-build.assert
```
