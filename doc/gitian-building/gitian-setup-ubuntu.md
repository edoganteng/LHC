...
The build script we'll be using is contained in the Lightcoin github repository ([contrib/gitian-build.py](https://github.com/LightcoinNodes/Lightcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Lightcoin repository and will need to fetch this script with the following commands:
...
- Clone the gitian-builder, gitian.sigs, Lightcoin-detached-sigs, and Lightcoin GitHub repos.
