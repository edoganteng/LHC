...
class ReorgStakeTest(LightcoinTestFramework):
    ...
    def check_money_supply(self, expected_lhc, expected_zLHC):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected LHC and zLHC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_lhc))
            for denom in node['zLightcoinupply']:
                assert_equal(node['zLightcoinupply'][denom], DecimalAmt(expected_zLHC[denom]))
    ...
    def run_test(self):
        ...
        # Check LHC and zLHC supply at the beginning
        # ------------------------------------------
        # zLHC supply: 2 coins for each denomination
        expected_zLHC_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # LHC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zLHC_supply)
        ...
        # Verify that LHC and zLHC supplies were properly updated after the spends and reorgs
        self.log.info("Check LHC and zLHC supply...")
        ...
        expected_zLHC_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zLHC_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zLHC_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zLHC_supply)
