...
class ZerocoinSpendTest(LightcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zLHC_bal, lhc_bal):
            zLHC_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zLHC_bal)
            lhc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], lhc_bal)
            return zLHC_bal, lhc_bal
        ...
        zLHC_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zLHC_balance, 6666)
        ...
        zLHC_balance, balance = check_balances(denom_2, zLHC_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zLHC_balance, balance = check_balances(denom_3, zLHC_balance, balance)
        ...
        _, _ = check_balances(denom_1, zLHC_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
