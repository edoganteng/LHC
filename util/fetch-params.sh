...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/LightcoinParams"
    else
        PARAMS_DIR="$HOME/.Lightcoin-params"
    fi
fi
...
Failed to fetch the Lightcoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Lightcoin - fetch-params.sh

This script will fetch the Lightcoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Lightcoin zkSNARK parameters. Note that it is
