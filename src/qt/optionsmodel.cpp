...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::LHC);
    ...
    if (settings.contains("nAnonymizeLightcoinAmount") || reset)
        SoftSetArg("-anonymizeLightcoinamount", settings.value("nAnonymizeLightcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Lightcoin.cpp during shotdown to also remove the window positions
