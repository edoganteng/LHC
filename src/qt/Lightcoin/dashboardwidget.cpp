...
#include "qt/Lightcoin/dashboardwidget.h"
#include "qt/Lightcoin/forms/ui_dashboardwidget.h"
#include "qt/Lightcoin/sendconfirmdialog.h"
#include "qt/Lightcoin/txrow.h"
...
DashboardWidget::DashboardWidget(LightcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareLHC, "square-chart-lhc");
    setCssProperty(ui->labelSquarezLHC, "square-chart-zLHC");
    setCssProperty(ui->labelLHC, "text-chart-lhc");
    setCssProperty(ui->labelzLHC, "text-chart-zLHC");
    ui->labelAmountzLHC->setVisible(false);
    ui->labelSquarezLHC->setVisible(false);
    ui->labelzLHC->setVisible(false);
    ...
    setCssProperty(ui->labelAmountLHC, "text-stake-lhc-disable");
    setCssProperty(ui->labelAmountzLHC, "text-stake-zLHC-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &LightcoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &LightcoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &LightcoinGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isLHC = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakezLHC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            lhc = (pair.first != 0) ? pair.first / 100000000 : 0;
            zLHC = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalLHC += pair.first;
            chartData->totalzLHC += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesLHC.append(lhc);
        chartData->valueszLHC.append(zLHC);

        int max = std::max(lhc, zLHC);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(0,76,218));
    set1->setColor(QColor(0,69,196));
    ...
    set0->append(chartData->valuesLHC);
    set1->append(chartData->valueszLHC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalLHC > 0 || chartData->totalzLHC > 0) {
        setCssProperty(ui->labelAmountLHC, "text-stake-lhc");
        setCssProperty(ui->labelAmountzLHC, "text-stake-zLHC");
    } else {
        setCssProperty(ui->labelAmountLHC, "text-stake-lhc-disable");
        setCssProperty(ui->labelAmountzLHC, "text-stake-zLHC-disable");
    }
    forceUpdateStyle({ui->labelAmountLHC, ui->labelAmountzLHC});
    ui->labelAmountLHC->setText(GUIUtil::formatBalance(chartData->totalLHC, nDisplayUnit));
    ui->labelAmountzLHC->setText(GUIUtil::formatBalance(chartData->totalzLHC, nDisplayUnit, true));

    series->append(set0);
    if (haszLightcointakes)
        series->append(set1);
