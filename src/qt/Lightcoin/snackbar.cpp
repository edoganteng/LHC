...
#include "qt/Lightcoin/forms/ui_snackbar.h"
#include "qt/Lightcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(LightcoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &LightcoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
