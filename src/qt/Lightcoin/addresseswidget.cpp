...
#include "qt/Lightcoin/forms/ui_addresseswidget.h"
#include "qt/Lightcoin/addresslabelrow.h"
#include "qt/Lightcoin/addnewaddressdialog.h"
#include "qt/Lightcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(LightcoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination lhcAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(lhcAdd, label.toUtf8().constData(),
