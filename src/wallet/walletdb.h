...
public:
    ...
    bool WriteZLightcoineed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZLightcoineed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZLightcoineed_deprecated(uint256& seed);
    bool EraseZLightcoineed();
    bool EraseZLightcoineed_deprecated();

    bool WritezLHCCount(const uint32_t& nCount);
    bool ReadzLHCCount(uint32_t& nCount);
