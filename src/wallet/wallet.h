...
#include "zLHC/zLHCmodule.h"
#include "zLHC/zLHCwallet.h"
#include "zLHC/zLHCtracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_Lightcoin = 61000, // inherited version..
...
enum ZerocoinSpendStatus {
    zLHC_SPEND_OKAY = 0,                            // No error
    zLHC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zLHC_WALLET_LOCKED = 2,                         // Wallet was locked
    zLHC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zLHC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zLHC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zLHC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zLHC_TRX_CREATE = 7,                            // Everything related to create the transaction
    zLHC_TRX_CHANGE = 8,                            // Everything related to transaction change
    zLHC_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    zLHC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zLHC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zLHC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zLHC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zLHC_SPENT_USED_zLHC = 14,                      // Coin has already been spend
    zLHC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    zLHC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void dozLHCRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckzLHC);
    ...
    bool CreatezLHCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPIV wallet
    std::unique_ptr<CzLHCTracker> zLHCTracker{nullptr};
    void setZWallet(CzLHCWallet* zwallet);
    CzLHCWallet* getZWallet();
