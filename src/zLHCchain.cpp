...
{
    CDataStream serializedCoinSpend = zLHCModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZLightcoinupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZLightcoinupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
