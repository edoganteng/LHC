...
const char * const Lightcoin_CONF_FILENAME = "Lightcoin.conf";
const char * const Lightcoin_PID_FILENAME = "Lightcoin.pid";
const char * const Lightcoin_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Unix: ~/.Lightcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LightcoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "LightcoinParams";
#else
    // Unix
    return pathRet / ".Lightcoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", Lightcoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", Lightcoin_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty Lightcoin.conf if it does not exist
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", Lightcoin_PID_FILENAME));
