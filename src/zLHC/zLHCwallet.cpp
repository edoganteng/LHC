...
CzLHCWallet::CzLHCWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLightcoineed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLightcoineed_deprecated()) {
                    LogPrintf("%s: Updated zLHC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zLHC seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zLHC wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLHCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezLHCCount(nCountLastUsed);
    else if (!walletdb.ReadzLHCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLHCWallet::Lock()
{
    seedMaster.SetNull();
}

void CzLHCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLHCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozLHC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLHCWallet::LoadMintPoolFromDB()
{
...
void CzLHCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLHCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLHCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zLHCTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLHCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozLHC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    wallet->zLHCTracker->Add(dMint, true);
...
void CzLHCWallet::SeedTozLHC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzLHCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLHCWallet::GenerateDeterministiczLHC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLHCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLHCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLHCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
