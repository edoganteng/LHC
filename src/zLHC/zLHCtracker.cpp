...
#include "zLHC/zLHCwallet.h"


CzLHCTracker::CzLHCTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzLHCTracker::~CzLHCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLHCTracker::Init()
{
...
bool CzLHCTracker::Archive(CMintMeta& meta)
{
...
bool CzLHCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLHCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLHCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLHCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLHCTracker::GetSerialHashes()
{
...
CAmount CzLHCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLHCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLHCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLHCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLHCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLHCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLHCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLHCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLHCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLHCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzLHCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLHCWallet* zLHCWallet)
{
    bool iszLHCWalletInitialized = (nullptr != zLHCWallet);
    CMintMeta meta;
    ...
    if (!iszLHCWalletInitialized)
        zLHCWallet = new CzLHCWallet(wallet);
    meta.isSeedCorrect = zLHCWallet->CheckSeed(dMint);
    if (!iszLHCWalletInitialized)
        delete zLHCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLHCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLHCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLHCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLHCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLHCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLHCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzLHC from DB\n", __func__, listDeterministicDB.size());
    }
