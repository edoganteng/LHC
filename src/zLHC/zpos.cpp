...
bool CLegacyZLightcointake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZLightcointake from non zc-spend", __func__);
    ...
    *this = CLegacyZLightcointake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zLHC stake origin", __func__);

    // All good
    return true;
}

CLegacyZLightcointake::CLegacyZLightcointake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZLightcointake::GetIndexFrom()
{
...
CAmount CLegacyZLightcointake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZLightcointake::GetUniqueness() const
{
...
bool CLegacyZLightcointake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zLHC stake block: height %d outside range", __func__, nHeight);
