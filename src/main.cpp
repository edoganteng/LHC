...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zLHC mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k LHC
    // Mainnet/Testnet block reward reduction schedule
    const int nLast = 8590000;
    if (nHeight > nLast)   return 21     * COIN;
    if (nHeight > 6590001)  return 23   * COIN;
    if (nHeight > 4590000)  return 23   * COIN;
    if (nHeight > 2590000)  return 24   * COIN;
    if (nHeight > 2070000)  return 25   * COIN;
    if (nHeight > 1550000)  return 26   * COIN;
    if (nHeight > 1450000)  return 27   * COIN;
    if (nHeight > 1350001)  return 28   * COIN;
    if (nHeight > 1250001)  return 29   * COIN;
    if (nHeight > 1150000)  return 30   * COIN;
    if (nHeight > 1060000)  return 31   * COIN;
    if (nHeight > 980000)  return 30.6   * COIN;
    if (nHeight > 910000)  return 30.4   * COIN;
    if (nHeight > 850000)  return 30.2   * COIN;
    if (nHeight > 800000)  return 30   * COIN;
    if (nHeight > 750000)  return 29.5   * COIN;
    if (nHeight > 710000)  return 29   * COIN;
    if (nHeight > 670000)  return 28.5   * COIN;
    if (nHeight > 630000)  return 28   * COIN;
    if (nHeight > 595000)  return 27.5   * COIN;
    if (nHeight > 560000)  return 27   * COIN;
    if (nHeight > 525000)  return 26.5   * COIN;
    if (nHeight > 490000)  return 26   * COIN;
    if (nHeight > 460000)  return 25.5   * COIN;
    if (nHeight > 430000)  return 25   * COIN;
    if (nHeight > 400000)  return 24.5   * COIN;
    if (nHeight > 370000)  return 24   * COIN;
    if (nHeight > 345000)  return 23.5   * COIN;
    if (nHeight > 320000)  return 23   * COIN;
    if (nHeight > 295000)  return 22.5  * COIN;
    if (nHeight > 270000)  return 21.5   * COIN;
    if (nHeight > 245000)  return 21  * COIN;
    if (nHeight > 220000)  return 20.5   * COIN;
    if (nHeight > 195000)  return 20  * COIN;
    if (nHeight > 170000)  return 19   * COIN;
    if (nHeight > 145000)  return 18  * COIN;
    if (nHeight > 120000)  return 17   * COIN;
    if (nHeight > 95000)   return 16   * COIN;
    if (nHeight > 70000)   return 13  * COIN;
    if (nHeight > 30000)   return 10  * COIN;
    if (nHeight > 1500)    return 2  * COIN;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!zLHCModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZLightcoinupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zLHC supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("Lightcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!zLHCModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    if (!UpdateZLightcoinupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zLHC supply for block=%s height=%d", __func__,
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateLightcoinupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zLHC serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Lightcoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nLightcoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nLightcoinBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> lhcInputs;
        std::vector<CTxIn> zLHCInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zLHCInputs.push_back(stakeIn);
            }else{
                lhcInputs.push_back(stakeIn);
            }
        }
        const bool hasLHCInputs = !lhcInputs.empty();
        const bool haszLHCInputs = !zLHCInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasLHCInputs) {
                    ...
                    for (const CTxIn& lhcIn : lhcInputs)
                        if(lhcIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            if(haszLHCInputs) {
                for (const CTxIn& zLHCInput : zLHCInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zLHCInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zLHC");
