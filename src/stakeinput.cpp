...
bool CLightcointake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLightcointake from zerocoin spend", __func__);
...
bool CLightcointake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CLightcointake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CLightcointake::GetTxOutFrom(CTxOut& out) const
{
...
bool CLightcointake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CLightcointake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CLightcointake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CLightcointake::GetUniqueness() const
{
...
CBlockIndex* CLightcointake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CLightcointake::ContextCheck(int nHeight, uint32_t nTime)
{
